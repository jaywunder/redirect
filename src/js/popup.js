  ...
  let view = new Vue({
    ...
    methods: {
      ...
      addSite (event) {
        ...
        if (this.distractions.filter(item => item.name === value).length >= 1) {
          return
        }
      ...
      removeSite (siteName) {
        this.distractions = this.distractions.filter(item => item.name !== siteName)
        this.updateConfig()
      },

      findIcon (siteName) {
        for (var url in this.siteIcons) {
          if (siteName.indexOf(url) >= 0) return this.siteIcons[url]
          return false
        }
      }
    }
